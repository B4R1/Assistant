...
def assistantResponse(text):
    ...
    # Convert the txt to speech
    myobj = gTTS(text=text, lang='en', slow=False)

    # Save the converted audio to a file
    myobj.save('assistant_respond.mp3')

    # Play the converted file
    os.system('start assistant_respond.mp3')

def wakeWord(text):
    WAKE_WORDS = ['hey computer', 'okay computer']
    text = text.lower() # Convert the text to all lower case
...
def getDate():
    ...
    return 'Today is ' + weekday + ' ' + month_names[monthNUM -1]
    + ' the ' + ordinalNumbers[dayNUM - 1] + '.'

def greeting(text):
    GREETING_INPUTS = ['hi', 'hey', 'hola', 'greetings',
    'wassup', 'hello']
